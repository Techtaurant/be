...
class CommentWriteService(
     ...
     */
    @Transactional
    fun createComment(userId: UUID, request: CreateCommentRequest): CommentResponse {
    ...
    private fun findPostById(postId: UUID) = postRepository.findById(postId).orElseThrow {
     ...
     *                     대댓글의 답글 시도
     */
    private fun resolveParent(parentId: UUID?, postId: UUID): Comment? {
        if (parentId == null) {
            return null
        }
