...
accessToken으로 Presigned URL 발급 API를 호출한다.
accessToken을 직접 넘기지 않으면 dev 로그인으로 자동 발급받는다.
...
옵션:
    ...
    --access-token TOKEN  직접 사용할 access token
...
예시:
    ...
    uv run test_presigned_upload.py --file ~/Downloads/pikachu.webp --type POST --base-url https://dev-api.techtaurant.com --access-token <your_access_token>
"""
...
def _issue_presigned_url(
    ...
    if not resp.ok:
        raise RuntimeError(
            f"Presigned URL 발급 실패 (HTTP {resp.status_code}): {resp.text}"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--file", help="업로드할 파일 경로 (생략 시 더미 PNG 자동 생성)"
    )
    parser.add_argument(
    ...
    parser.add_argument("--access-token", help="직접 사용할 access token")
    parser.add_argument(
    ...
    try:
        if args.access_token:
            print("\n▶ Step 1. 전달받은 access token 사용...")
            access_token = args.access_token
            print("   ✅ access token 적용 완료")
        else:
