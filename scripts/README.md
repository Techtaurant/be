...
| 파일 | 역할 |
|------|------|
| `test_presigned_upload.py` | dev 로그인 API 호출 + Presigned URL 발급 + S3 업로드 |

### 수행 단계

1. `POST /open-api/dev/auth/login` → accessToken 발급
...
# 모든 옵션을 함께 사용
uv run test_presigned_upload.py --file ~/Downloads/pikachu.webp --type POST --base-url http://localhost:8080 --identifier dev-test-user
```
...
▶ Step 1. Dev 로그인 API 호출 (test-uploader)...
