...
def get_http_status_code(status: BaseStatus) -> int:
    """
    BaseStatus에 해당하는 HTTP 상태 코드를 반환합니다.
    ...
    Example:
        ```python
        code = get_http_status_code(BaseStatus.SUCCESS)
        print(code)  # 200
        ```
    """
...
def get_default_message(status: BaseStatus) -> str:
    """
    BaseStatus에 해당하는 기본 메시지를 반환합니다.
    ...
    Example:
        ```python
        message = get_default_message(BaseStatus.SUCCESS)
        print(message)  # "요청이 성공했습니다"
        ```
    """
